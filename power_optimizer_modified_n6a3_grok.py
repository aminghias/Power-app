...
class PowerOptimizer:
    ...
    def optimize_power_allocation_cost(self):
        ...
        # NEW: Balancing after allocation and sharing
        total_allocated_cost_opt_load = sum(s.optimized_cost_active_load for s in available_sources)
        print(f"Total allocated cost optimized load: {total_allocated_cost_opt_load:.2f} kW")

        load_difference = remaining_active_load - total_allocated_cost_opt_load
        deficit_load = load_difference if load_difference > 0 else 0

        # Check and fix sources exceeding max_capacity
        for source in available_sources:
            if source.optimized_cost_active_load > source.max_capacity:
                excess = source.optimized_cost_active_load - source.max_capacity
                source.optimized_cost_active_load = source.max_capacity
                deficit_load += excess
                print(f"Source {source.name} exceeded max_capacity. Reduced to {source.max_capacity:.2f} kW. Excess: {excess:.2f} kW")
        
        # Redistribute deficit_load
        if deficit_load > 0:
            print(f"Redistributing deficit load: {deficit_load:.2f} kW")
            
            # Step 1: Try to assign to machines with zero active power
            zero_load_sources = [s for s in available_sources if s.optimized_cost_active_load == 0 and s.max_capacity > 0]
            zero_load_sources.sort(key=lambda x: x.efficiency_score)

            for source in zero_load_sources:
                if deficit_load <= 0:
                    break
                max_possible = min(deficit_load, source.max_capacity)
                if max_possible >= source.min_capacity:
                    assignable = max_possible
                    source.optimized_cost_active_load = assignable
                    deficit_load -= assignable
                    print(f"Assigned {assignable:.2f} kW to previously unused source {source.name}")
            
            # Step 2: Try to assign remaining to machines that still have capacity
            if deficit_load > 0:
                running_sources = [s for s in available_sources if s.optimized_cost_active_load > 0]
                running_sources.sort(key=lambda x: x.efficiency_score)
                for source in running_sources:
                    if deficit_load <= 0:
                        break
                    available_capacity = source.max_capacity - source.optimized_cost_active_load
                    if available_capacity > 0:
                        assignable = min(deficit_load, available_capacity)
                        source.optimized_cost_active_load += assignable
                        deficit_load -= assignable
                        print(f"Assigned additional {assignable:.2f} kW to source {source.name}")
            ...
            if deficit_load > 0:
                for bess in self.bess_systems:
                    if deficit_load <= 0:
                        break
                    bess.check_availability()
                    if bess.available and bess.can_charge(deficit_load):
                        assignable = min(deficit_load, bess.power_rating_kw - bess.optimized_charge_power)
                        bess.optimized_charge_power += assignable
                        bess.mode = 'charging' if bess.optimized_charge_power > 0 else bess.mode
                        deficit_load -= assignable
                        print(f"Assigned {assignable:.2f} kW to BESS {bess.name} for charging")
            ...
            # Step 4: Try to assign to grid feed
            if deficit_load > 0 and self.allow_grid_feed:
                grid_feed_capacity = self.grid_feed_limit - self.grid_feed
                if grid_feed_capacity > 0:
                    assignable = min(deficit_load, grid_feed_capacity)
                    self.grid_feed += assignable
                    deficit_load -= assignable
                    print(f"Fed {assignable:.2f} kW to grid")
            ...
            # Step 5: Report any remaining unassigned load
            if deficit_load > 0:
                print(f"WARNING: Unable to assign {deficit_load:.2f} kW of load. System capacity insufficient.")
        ...
        total_allocated = sum(s.optimized_cost_active_load for s in self.sources)
        excess = total_allocated - remaining_active_load
        if excess > 0:
            # Try to charge BESS with excess
            for bess in self.bess_systems:
    ...
    def handle_off_grid_operation_cost(self, sources):
        ...
        if total_generation < total_demand:
            ...
            for source in sources:
                if source.optimized_cost_active_load < source.max_capacity:
                    additional_capacity = min(deficit, 
                                              source.max_capacity - source.optimized_cost_active_load)
                    source.optimized_cost_active_load += additional_capacity
                    deficit -= additional_capacity
                    print(f"Increased {source.name} output by {additional_capacity:.2f} kW")
            ...
            if deficit > 0:
                print(f"Load shedding required: {deficit:.2f} kW")
                # Removed incorrect reduction loop; just note shedding
    ...
    def optimize_power_allocation_reliability(self):
        ...
        # find difference
        load_difference = remaining_active_load - total_allocated_rel_opt_load
        ...
        if deficit_load > 0:
            ...
            for source in zero_load_sources:
                if deficit_load <= 0:
                    break
                max_possible = min(deficit_load, source.effective_max)
                if max_possible >= source.min_capacity:
                    assignable = max_possible
                    source.optimized_rel_active_load = assignable
                    deficit_load -= assignable
            ...
            if deficit_load > 0:
                for source in available_sources:
                    ...
                    available_capacity = source.effective_max - source.optimized_rel_active_load
                    if available_capacity > 0:
                        if source.optimized_rel_active_load > 0:
                            assignable = min(deficit_load, available_capacity)
                        else:
                            max_possible = min(deficit_load, available_capacity)
                            if max_possible >= source.min_capacity:
                                assignable = max_possible
                            else:
                                continue
...
#         return '\n\n'.join(recommendations)
    ...
    def generate_recommendations(self, results_df):
        ...
        total_cost_optimized_load = results_df[results_df['ENERGY SOURCE'] == 'TOTAL']['COST OPTIMIZED LOAD (kW)'].iloc[0]
        ...
        # Overall summary
        total_row = results_df[results_df['ENERGY SOURCE'] == 'TOTAL'].iloc[0]
        optimized_load_key = 'COST OPTIMIZED LOAD (kW)' if use_cost_optimization else 'RELIABILITY OPTIMIZED LOAD (kW)'
        ...
        summary = f"""
    **OPTIMIZATION SUMMARY**:
    • Total current load: {total_row['CURRENT LOAD (kW)']:.2f} kW
    ...
    • Optimization prioritizes {'lowest total cost' if use_cost_optimization else 'reliability'} with reliability
    """
