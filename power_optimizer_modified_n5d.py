...
class PowerSource:
    ...
    def calculate_current_cost(self):
        ...
        # Calculate cost from power sources
        for source in self.power_sources.values():
            active_power = source.current_active_load if source.available else 0.0  # Use current_active_load from PowerSource
            production_cost = source.total_cost
            total_cost += active_power * production_cost
        
        # Calculate cost from BESS (if charging or discharging)
        for bess in self.bess_systems_dict.values():
            power_input = bess.current_power_input
            production_cost = bess.production_cost
            # Cost is incurred only when power is used (positive for charging, negative for discharging)
...
class PowerOptimizer:
    ...
    def initialize_sources(self, site_id):
        ...
        self.sources = []
        self.power_sources = {}  # Added dictionary for calculate_current_cost compatibility
        ...
        for config in sources_config:
            ...
            self.power_sources[source.name] = source  # Store as dictionary with name as key
        
        bess_config = self.load_bess_config(site_id)
        self.bess_systems = []
        self.bess_systems_dict = {}  # Added dictionary for BESS compatibility
        ...
        for config in bess_config:
            ...
            self.bess_systems_dict[bess.name] = bess  # Store as dictionary with name as key
    ...
    def calculate_current_cost(self):
        ...
        # Calculate cost from power sources
        for source in self.power_sources.values():
            active_power = source.current_active_load if source.available else 0.0  # Use current_active_load
            production_cost = source.total_cost  # Use total_cost including carbon
            total_cost += active_power * production_cost
        
        # Calculate cost from BESS (if charging or discharging)
        for bess in self.bess_systems_dict.values():  # Use bess_systems_dict as defined in initialize_sources
            power_input = bess.current_power_input
            production_cost = bess.production_cost
            # Cost is incurred only when power is used (positive for charging, negative for discharging)
            if power_input != 0 and bess.available:
    ...
    # cost_optimized_loads

    def optimize_power_allocation(self):
        """Optimize power allocation based on cost or reliability criteria."""
        # Calculate initial costs and losses
        current_cost = self.calculate_current_cost()
        self.optimize_power_allocation_cost()  # Populate cost-optimized results
        cost_optimized_cost = self.calculate_optimized_cost() 
        cost_diff = current_cost - cost_optimized_cost

        self.optimize_power_allocation_reliability()  # Populate reliability-optimized results
        rel_optimized_cost = self.calculate_optimized_cost()
        total_loss = self.calculate_total_loss()

        # Decide optimization mode based on cost savings vs. reliability loss
        if cost_diff > total_loss:
            self.optimized_mode = 'cost'
            # Ensure cost-optimized results are used
            ...
            # Ensure reliability-optimized results are used
        ...
        # Update the internal state with the chosen optimization
    ...
    def optimize_power_allocation_cost(self):
        ...
        self.allocate_reactive_power(kvar_sources, total_reactive_load, mode='cost')
    ...
    def optimize_power_allocation_reliability(self):
        ...
        self.allocate_reactive_power(kvar_sources, total_reactive_load, mode='reliability')
        
        # Handle excess if any
        remaining_active_load = total_active_load - sum(b.rel_optimized_discharge_power for b in self.bess_systems)
        total_allocated = sum(s.rel_optimized_active_load for s in self.sources)
        ...
        if excess > 0:
            ...
            for bess in self.bess_systems:
                ...
                if bess.available and bess.can_charge(excess):
                    bess.rel_optimized_charge_power += excess
                    if bess.rel_optimized_discharge_power > 0:
                        bess.rel_optimized_discharge_power = max(0, bess.rel_optimized_discharge_power - excess)
            ...
            if excess > 0 and self.allow_grid_feed:
                self.rel_grid_feed = min(excess, self.grid_feed_limit)
                for s in self.sources:
                    if s.name.lower() == 'grid':
                        s.rel_grid_feed_power = self.rel_grid_feed
                        break
    ...
    def generate_results(self):
        ...
        rel_savings_hr = total_current_cost - total_rel_optimized_cost
        results.append({
            'ENERGY SOURCE': 'TOTAL',
            ...
            'RELIABILITY SCORE': '',
            'EFFICIENCY SCORE': '',
            'STATUS': f'Cost Savings: PKR {cost_savings_hr:.2f}/hr, Rel Savings: PKR {rel_savings_hr:.2f}/hr'
        })
        ...
        # Choose which optimization to use based on total loss vs cost difference
        ...
        if total_loss > cost_diff:
            ...
            total_savings_hr = rel_savings_hr
        else:
            self.optimized_mode = 'cost'
            for source in self.sources:
            ...
            total_savings_hr = cost_savings_hr
    ...
    def generate_cost_results(self):
        ...
        for source in self.sources:
            ...
            row = {
                ...
                'OPTIMIZED LOAD (kW)': effective_optimized,
                'CURRENT KVAR (kVAR)': current_kvar,
                'OPTIMIZED KVAR (kVAR)': optimized_kvar,
                'TOTAL COST (PKR/kWh)': cost_per_kwh,
                'PRODUCTION COST (PKR/kWh)': source.production_cost,
                'CARBON COST (PKR/kWh)': source.carbon_emission * self.carbon_cost_pkr_kg,
                'CURRENT COST/HR': current_cost_hr,
                'OPTIMIZED COST/HR': optimized_cost_hr,
                'OPTIMIZED CHARGE (kW)': 0.0,
                'OPTIMIZED DISCHARGE (kW)': 0.0,
                'GRID FEED (kW)': source.cost_grid_feed_power,
                'RELIABILITY SCORE': source.reliability_score,
                'EFFICIENCY SCORE': source.efficiency_score,
                'STATUS': 'Active' if source.available else 'Inactive'
            }
        ...
        for bess in self.bess_systems:
            ...
            row = {
                'ENERGY SOURCE': bess.name,
                'CURRENT LOAD (kW)': current_bess_discharge,
                'OPTIMIZED LOAD (kW)': optimized_bess_load,
                'CURRENT KVAR (kVAR)': current_kvar,
                'OPTIMIZED KVAR (kVAR)': optimized_kvar,
                'TOTAL COST (PKR/kWh)': bess.total_cost,
                'PRODUCTION COST (PKR/kWh)': bess.production_cost,
                'CARBON COST (PKR/kWh)': bess.carbon_emission * self.carbon_cost_pkr_kg,
                'CURRENT COST/HR': current_cost_hr,
                'OPTIMIZED COST/HR': optimized_cost_hr,
                'OPTIMIZED CHARGE (kW)': bess.cost_optimized_charge_power,
                'OPTIMIZED DISCHARGE (kW)': bess.cost_optimized_discharge_power,
                'GRID FEED (kW)': 0.0,
                'RELIABILITY SCORE': bess.reliability_score,
                'EFFICIENCY SCORE': 0,  # Placeholder
                'STATUS': status
            }
        ...
        results.append({
            'ENERGY SOURCE': 'TOTAL',
            'CURRENT LOAD (kW)': total_current_load,
            'OPTIMIZED LOAD (kW)': total_optimized_load,
            'CURRENT KVAR (kVAR)': total_current_kvar,
            'OPTIMIZED KVAR (kVAR)': total_optimized_kvar,
            'TOTAL COST (PKR/kWh)': '',
            'PRODUCTION COST (PKR/kWh)': '',
            'CARBON COST (PKR/kWh)': '',
            'CURRENT COST/HR': total_current_cost,
            'OPTIMIZED COST/HR': total_optimized_cost,
            'OPTIMIZED CHARGE (kW)': sum(b.cost_optimized_charge_power for b in self.bess_systems),
            'OPTIMIZED DISCHARGE (kW)': sum(b.cost_optimized_discharge_power for b in self.bess_systems),
            'GRID FEED (kW)': total_grid_feed,
            'RELIABILITY SCORE': '',
            'EFFICIENCY SCORE': '',
    ...
    def generate_reliability_results(self):
        ...
        for source in self.sources:
            ...
            row = {
                ...
                'OPTIMIZED CHARGE (kW)': 0.0,
                'OPTIMIZED DISCHARGE (kW)': 0.0,
                'GRID FEED (kW)': source.rel_grid_feed_power,
                'RELIABILITY SCORE': source.reliability_score,
                'EFFICIENCY SCORE': source.efficiency_score,
                'STATUS': 'Active' if source.available else 'Inactive'
            }
        ...
        for bess in self.bess_systems:
            ...
            row = {
                ...
                'CURRENT COST/HR': current_cost_hr,
                'OPTIMIZED COST/HR': optimized_cost_hr,
                'OPTIMIZED CHARGE (kW)': bess.rel_optimized_charge_power,
                'OPTIMIZED DISCHARGE (kW)': bess.rel_optimized_discharge_power,
                'GRID FEED (kW)': 0.0,
                'RELIABILITY SCORE': bess.reliability_score,
                'EFFICIENCY SCORE': 0,  # Placeholder
                'STATUS': status
        ...
        results.append({
            ...
            'CURRENT COST/HR': total_current_cost,
            'OPTIMIZED COST/HR': total_optimized_cost,
            'OPTIMIZED CHARGE (kW)': sum(b.rel_optimized_charge_power for b in self.bess_systems),
            'OPTIMIZED DISCHARGE (kW)': sum(b.rel_optimized_discharge_power for b in self.bess_systems),
            'GRID FEED (kW)': total_grid_feed,
            'RELIABILITY SCORE': '',
            'EFFICIENCY SCORE': '',
            'STATUS': f'Savings: PKR {total_savings_hr:.2f}/hr'
    ...
    def generate_recommendations(self, results_df):
        ...
        for _, row in results_df.iterrows():
            ...
            cost_optimized_kw = row['COST OPT LOAD (kW)']
            rel_optimized_kw = row['REL OPT LOAD (kW)']
            ...
            if 'BESS' in source:
                ...
                rec += f"• Cost Optimization:\n"
                if cost_charge_kw > 0:
                    rec += f"  - Charge at {cost_charge_kw:.1f} kW\n"
                elif cost_discharge_kw > 0:
                    rec += f"  - Discharge at {cost_discharge_kw:.1f} kW\n"
                else:
                    rec += f"  - Maintain standby mode\n"
                
                rec += f"• Reliability Optimization:\n"
                if rel_charge_kw > 0:
                    rec += f"  - Charge at {rel_charge_kw:.1f} kW\n"
                elif rel_discharge_kw > 0:
                    rec += f"  - Discharge at {rel_discharge_kw:.1f} kW\n"
                else:
                    rec += f"  - Maintain standby mode\n"
                ...
                rec += f"• Cost Optimization:\n"
                if cost_optimized_kw > current_kw:
                    rec += f"  - Increase load from {current_kw:.1f} kW to {cost_optimized_kw:.1f} kW\n"
                elif cost_optimized_kw < current_kw:
                    rec += f"  - Reduce load from {current_kw:.1f} kW to {cost_optimized_kw:.1f} kW\n"
                else:
                    rec += f"  - Maintain current load of {current_kw:.1f} kW\n"
                
                rec += f"• Reliability Optimization:\n"
                if rel_optimized_kw > current_kw:
                    rec += f"  - Increase load from {current_kw:.1f} kW to {rel_optimized_kw:.1f} kW\n"
                elif rel_optimized_kw < current_kw:
                    rec += f"  - Reduce load from {current_kw:.1f} kW to {rel_optimized_kw:.1f} kW\n"
                else:
                    rec += f"  - Maintain current load of {current_kw:.1f} kW\n"
...
• Cost-optimized load: {total_row['COST OPT LOAD (kW)']} kW
• Reliability-optimized load: {total_row['REL OPT LOAD (kW)']} kW
• Current hourly cost: PKR {total_row['CURRENT COST/HR']:,.2f}
• Cost-optimized hourly cost: PKR {total_row['COST OPT COST/HR']:,.2f}
• Reliability-optimized hourly cost: PKR {total_row['REL OPT COST/HR']:,.2f}
• Cost optimization savings: PKR {total_row['CURRENT COST/HR'] - total_row['COST OPT COST/HR']:,.2f}
• Reliability optimization savings: PKR {total_row['CURRENT COST/HR'] - total_row['REL OPT COST/HR']:,.2f}
• Total cost-optimized BESS charge: {total_row['COST OPT CHARGE (kW)']} kW
• Total cost-optimized BESS discharge: {total_row['COST OPT DISCHARGE (kW)']} kW
• Total reliability-optimized BESS charge: {total_row['REL OPT CHARGE (kW)']} kW
• Total reliability-optimized BESS discharge: {total_row['REL OPT DISCHARGE (kW)']} kW
• Cost-optimized grid feed: {total_row['COST GRID FEED (kW)']} kW
• Reliability-optimized grid feed: {total_row['REL GRID FEED (kW)']} kW
• Chosen optimization mode: {self.optimized_mode}
        """
