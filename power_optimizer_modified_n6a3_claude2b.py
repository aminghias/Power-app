...
class PowerOptimizer:
    ...
    def optimize_power_allocation_reliability(self):
        ...
        self.standby_dispatchable = total_loss > self.standby_threshold
        if total_loss > self.very_high_threshold:
            self.redundancy_level = 2
            self.prefer_bess = True and not self.standby_dispatchable
        ...
        if self.standby_dispatchable:
            available_sources = [s for s in available_sources if s.source_type != 'grid']
        ...
        if self.standby_dispatchable:
            kvar_sources = [s for s in kvar_sources if not (hasattr(s, 'source_type') and s.source_type == 'grid') and not isinstance(s, BatteryEnergyStorageSystem)]
    ...
    def allocate_additional_load_rel(self, sources, additional_load):
        ...
        if remaining > 0:
            if not self.standby_dispatchable:
                for bess in self.bess_systems:
                    if remaining <= 0:
                        break
                    bess.check_availability()
        ...
        if remaining > 0 and self.allow_grid_feed and not self.standby_dispatchable:
