...
def main():
    ...
    st.sidebar.subheader("Reliability & Reserve Settings")
    reserve_percent = st.sidebar.number_input("Reserve Percentage (%)", min_value=0.0, max_value=100.0, value=25.0, help="Spinning reserve for reliability")
    plant_max_load = st.sidebar.number_input("Plant Max Load (kW)", min_value=0.0, value=5000.0, help="Maximum expected plant load for redundancy checks")
    power_failure_cost = st.sidebar.number_input("Power Failure Cost (PKR/min)", min_value=0.0, value=1000.0, help="Cost of downtime to decide reserve type")
        ...
        if st.button("🚀 Run Optimization", type="primary", use_container_width=True):
            with st.spinner("Running power optimization..."):
                try:
                    ...
                    optimizer.set_reserve_params(
                        reserve_percent=reserve_percent,
                        plant_max_load=plant_max_load,
